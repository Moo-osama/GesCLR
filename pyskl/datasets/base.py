...
class BaseDataset(Dataset, metaclass=ABCMeta):
        ...
        **deprecated_kwargs,
        ...
        if isinstance(results[0], list) or isinstance(results[0], tuple):
            ...
            for i in range(num_results):
                ...
                eval_results.update(
                    {f"{k}_{i}": v for k, v in eval_results_cur.items()}
                )
            return eval_results
        ...
        with open("/home/osabdelfattah/MotionBert_k400/gt_labels.txt", "w") as file:
            file.write(str(gt_labels))
        for metric in metrics:
            ...
            if metric == "confusion_matrix":
                ...
                eval_results["confusion_matrix"] = cf_mat
                log_msg = f"\nconfusion_matrix\t{cf_mat:.4f}"
                print_log(log_msg, logger=logger)
    ...
    def prepare_train_frames(self, idx):
        ...
        if self.memcached and "key" in results:
            ...
            if not isinstance(pack, dict):
                ...
                raw_file = self.data_prefix + raw_file
                data = mmcv.load(raw_file)
                (head, key) = osp.split(key)
                pack = data[key]
                for k in data:
                    try:
    ...
    def __getitem__(self, idx):
        ...
        return (
            self.prepare_test_frames(idx)
            if self.test_mode
            else self.prepare_train_frames(idx)
        )
