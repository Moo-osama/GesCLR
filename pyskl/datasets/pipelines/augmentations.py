...
class ActionDataset(Dataset):
    ...
    def __call__(self, results):
        resample_id = resample(
            ...
            target_len=self.n_frames,
            randomness=self.is_train,
        )
        motion_cam = make_cam(x=results["keypoint"], img_shape=results["img_shape"])
        motion_cam = human_tracking(motion_cam)
        motion_cam = coco2h36m(motion_cam)
        motion_conf = results["keypoint_score"][..., None]
        self.motions = np.concatenate(
            (motion_cam[:, resample_id], motion_conf[:, resample_id]), axis=-1
        )

        results["keypoint"] = self.motions
