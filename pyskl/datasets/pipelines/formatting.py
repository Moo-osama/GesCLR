...
class ToTensor:
    ...
    def __init__(self, keys, max_person=10):
        self.keys = keys
    ...
    def __call__(self, results):
        ...
        for key in self.keys:
            results[key] = to_tensor(results[key]).to(torch.device("cpu"))
        x = results["keypoint"]

        results["keypoint"] = torch.cat(
            (
                x,
                torch.zeros(
                    self.max_person - x.size(0), x.size(1), x.size(2), x.size(3)
                ),
            ),
            dim=0,
        )
