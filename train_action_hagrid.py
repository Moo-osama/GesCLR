...
from lib.data.dataset_action import HagridActionDataset
...
def validate(test_loader, model, criterion):
    ...
    with torch.no_grad():
        ...
        for idx, (batch_input, batch_gt) in tqdm(enumerate(test_loader)):
            ...
            # Collect predicted labels and ground truth labels
            _, predicted = output.max(1)
            all_predictions.extend(predicted.cpu().numpy())
            all_targets.extend(batch_gt.cpu().numpy())
    ...
    return losses.avg, top1.avg, top5.avg, conf_matrix
...
def train_with_config(args, opts):
    ...
    data_path = "hagrid_full.pkl"
    hagrid_train = HagridActionDataset(
        data_path=data_path,
        data_split="train",
    ...
    hagrid_val = HagridActionDataset(
        data_path=data_path,
        data_split="test",
        n_frames=args.clip_len,
        random_move=False,
    ...
    train_loader = DataLoader(hagrid_train, **trainloader_params)
    test_loader = DataLoader(hagrid_val, **testloader_params)
    ...
    if opts.evaluate:
        test_loss, test_top1, test_top5, conf_matrix = validate(
        ...
        class_names = [class_name for class_name in labels_dict.keys()]
        class_accuracy = np.column_stack((class_names, per_class_accuracy))
        np.savetxt("per_class_accuracy.csv", class_accuracy, fmt="%s", delimiter=",")

        conf_matrix_named = np.zeros((len(class_names), len(class_names)), dtype=int)
