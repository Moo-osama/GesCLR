...
def human_tracking(x):
    ...
    if M == 1:
        ...
        diff0 = np.sum(
            np.linalg.norm(x[0, 1:] - x[0, :-1], axis=-1), axis=-1
        )  # (T-1, V, C) -> (T-1)
    ...
    def __init__(
        ...
        for sample in annotations:
            ...
            resample_id = resample(
                ori_len=sample["total_frames"],
                target_len=n_frames,
                randomness=self.is_train,
            )
            ...
            motion = np.concatenate(
                (motion_cam[:, resample_id], motion_conf[:, resample_id]), axis=-1
            )
...
class NTURGBD(ActionDataset):
    def __init__(
        ...
        super(NTURGBD, self).__init__(
            data_path, data_split, n_frames, random_move, scale_range
        )
...
class NTURGBD1Shot(ActionDataset):
    def __init__(
        ...
        super(NTURGBD1Shot, self).__init__(
            data_path, data_split, n_frames, random_move, scale_range, check_split
        )
        oneshot_classes = [
            0,
            6,
...
class k400ActionDataset(Dataset):
    def __init__(
        self, data, n_frames=243, random_move=True, scale_range=[1, 1], check_split=True
    ):  # data_split: train/test etc.
        np.random.seed(0)
...
class K400(ActionDataset):
    def __init__(self, n_frames=243, random_move=True, scale_range=[1, 1]):
        super(K400, self).__init__(n_frames, random_move, scale_range)
...
class HagridActionDataset(Dataset):
    def __init__(
        self,
        data_path,
        data_split,
        ...
        for sample in tqdm(annotations):
            ...
            # )
            motion_cam = make_cam(x=sample["keypoint"], img_shape=sample["img_shape"])
            if motion_cam.shape[2] == 0:
                continue
            # motion_cam = human_tracking(motion_cam)
            # motion_cam = coco2h36m(motion_cam)
            motion_conf = sample["keypoint_score"][..., None]
            # print("conf", motion_conf.shape)
            # print("cam", motion_cam.shape)
            motion = np.concatenate((motion_cam, motion_conf), axis=-1)
            # if motion.shape[0]==1:                                  # Single person, make a fake zero person
            #     fake = np.zeros(motion.shape)
            #     motion = np.concatenate((motion, fake), axis=0)
            motions.append(motion.astype(np.float32))
